...
قسم مستقل لدفتر إنجازات **الصف الرابع الابتدائي**: الطالبة تختار مادة، تحكي ما أنجزته فيها، فتكسب نجمة — وعند اكتمال نجوم المادة يظهر تاجها. الدفتر يعيش في `localStorage` فقط ولا يمسّ Firebase ولا لوحة المعلمة.

- **مصدر التصميم الوحيد**: `src/injazi/styles/tokens.css` (ألوان، ظلال، مسافات، منحنيات ومدد الحركة) و`src/injazi/motion/motion.ts` (متغيّرات الحركة المشتركة). لا قيمة لونية أو منحنى حركة يُكتب يدويًا داخل مكوّن.
- **عائلة بصرية واحدة**: كل الأجسام (كتاب، قلم، نجمة، حقيبة، آلة حاسبة، مجهر، كرة أرضية، تاج) من `ClayObject.tsx` بالقواعد نفسها، ويقرأ المشهد ثلاثي الأبعاد ألوانه من رموز CSS ذاتها.
- **RTL أصلي**: خصائص منطقية (`inline-start/end`) في كل الأنماط، وإزاحات الحركة الأفقية عبر `inlineX()` فتنقلب مع اتجاه الصفحة.
- **الأداء**: `/injazi` كله محمَّل بـ lazy، و`three` و`lottie-web` في حزمتين منفصلتين لا تُطلبان إلا عند الحاجة. المشهد ثلاثي الأبعاد لا يُحمَّل على الهاتف ولا على الأجهزة الضعيفة ولا عند `prefers-reduced-motion` — والبديل المسطّح كامل الوظيفة. بوابة القرار في `src/injazi/lib/useCapability.ts`.
- **دخول الفراشات**: يظهر في أول زيارة فقط (علامة في `localStorage`)، طبقة `pointer-events: none` لا تحجب شيئًا، ويُلغى كليًا عند تقليل الحركة.
...
الآن `/teacher` تعمل باسم مستخدم `Dalal` / كلمة مرور `Dalal1234` (لا بريد إلكتروني ظاهر في الواجهة إطلاقًا - انظر "دخول المعلمة" أدناه) و`/play` تعرض الأسئلة الـ16 المزروعة - كل ذلك محليًا بلا أي اتصال بـ Firebase الحقيقي. واجهة فحص المحاكي: `http://127.0.0.1:4000`.
